...
mapLoop:
    # add t1, s0, x0      # load the address of the array of current node into t1, t1 = array
    lw t1 ,0(s0)
    lw t2, 4(s0)        # load the size of the node's array into t2, t2 = size
    

    slli t3, t0, 2
    # add t1, t1, t0      # offset the array address by the count
    add t1, t1, t3      # t1 表示当前数组中的一个元素地址
    lw a0, 0(t1)        # load the value at that address into a0

    addi sp, sp, -16    #  暂存t0, t1, t2, t3
    sw t0, 0(sp)    
    sw t1, 4(sp)
    sw t2, 8(sp)
    sw t3, 12(sp)
    ...
    lw a0, 8(s0)
    # addi a0, s0, 8
    # la a1, 0(s1)        # put the address of the function back into a1 to prepare for the recursion
    mv a1, s1
